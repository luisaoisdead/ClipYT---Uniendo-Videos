...
class AppEstudioEdicionClipYT(TkinterDnD.Tk):
    def __init__(self):
        ...
        # Referencias directas a los componentes Listbox de la UI
        self.lista_v_ui = None
        self.lista_a2_ui = None
        self.lista_a3_ui = None
        
        # Variables de control para el Slider de tiempo
        ...
        self.lbl_reloj = ctk.CTkLabel(self.frame_timeline, text="00:00 / 00:00", font=ctk.CTkFont(family="Arial", size=12, weight="bold"))
    ...
    def matar_reproductor_activo(self):
        ...
        try:
            subprocess.run(['taskkill', '/f', '/im', 'ffplay.exe'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except:
            pass
        ...
        while self.reproduciendo:
            if self.tiempo_actual < self.duracion_total:
                ...
                if self.proceso_player and self.proceso_player.poll() is not None:
                    self.reproduciendo = False
                    self.after(0, lambda: self.slider_timeline.set(0))
                    break
                ...
                self.after(0, lambda curr=self.tiempo_actual: self.slider_timeline.set(curr) if self.reproduciendo else None)
                self.after(0, lambda text=f"{min_cur:02d}:{seg_cur:02d} / {min_tot:02d}:{seg_tot:02d}": self.lbl_reloj.configure(text=text))
            else:
                self.reproduciendo = False
                break
    ...
    def previsualizar_en_vivo_worker(self):
        ...
        try:
            ...
            self.after(0, lambda: self.slider_timeline.set(0))
            self.after(0, lambda: self.lbl_reloj.configure(text=f"00:00 / {min_tot:02d}:{seg_tot:02d}"))
            ...
            self.after(0, lambda: messagebox.showerror("Error", f"Problema con el reproductor.\nDetalle: {str(e)}"))
        finally:
    ...
    def lanzar_ffplay_en_second_worker(self, segundos, archivo):
        ...
        comando_play = [
            ...
            '-window_title', f'ClipYT Player (Total: {str_total}) | [ESPACIO = Pausa | Arrastrá la barra para saltar]',
            archivo
        ]
    ...
    def guardar_render_final(self):
        ...
        try:
            ...
            if os.path.exists("temp_preview_clipyt.mp4"):
                try: os.remove("temp_preview_clipyt.mp4")
                except: pass
